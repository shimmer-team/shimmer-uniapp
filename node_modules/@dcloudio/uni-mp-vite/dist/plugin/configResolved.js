...
function createConfigResolved({ cdn, style: { extname }, template: { component }, }) {
    ...
    return (config) => {
        ...
        const unocssGlobalBuildGenerateIndex = config.plugins.findIndex((p) => p.name === 'unocss:global:build:generate');
        ...
        if (hasUnocssGlobalBuildGenerate && extname !== '.css') {
            ;
            config.plugins.splice(unocssGlobalBuildGenerateIndex + 1, 0, adjustCssExtname(extname));
        }
    };
}
