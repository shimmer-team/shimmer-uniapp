...
function processComponent(node, context) {
    ...
    if ((0, uni_shared_1.isComponentTag)(tag)) {
        return context.onError((0, errors_1.createMPCompilerError)(8 /* X_DYNAMIC_COMPONENT_NOT_SUPPORTED */, node.loc));
    }
    if ((0, compiler_core_1.findDir)(node, 'is')) {
        return context.onError((0, errors_1.createMPCompilerError)(6 /* X_V_IS_NOT_SUPPORTED */, node.loc));
    }
    // TODO not supported
    ...
    if (builtIn) {
        return context.onError((0, errors_1.createMPCompilerError)(7 /* X_NOT_SUPPORTED */, node.loc, tag));
    }
...
function processProps(node, context, props = node.props) {
    ...
    for (let i = 0; i < props.length; i++) {
        ...
        if (prop.type === 7 /* DIRECTIVE */) {
            ...
            if (name === 'slot') {
                if (!isComponent) {
                    context.onError((0, compiler_core_1.createCompilerError)(40 /* X_V_SLOT_MISPLACED */, loc));
                }
                continue;
            }
            ...
            if (isVBind || isVOn) {
                ...
                // v-bind=""
                if (!arg) {
                    context.onError((0, errors_1.createMPCompilerError)(isVBind
                        ? 2 /* X_V_BIND_NO_ARGUMENT */
                        : 0 /* X_V_ON_NO_ARGUMENT */, loc));
                    continue;
                }
                // v-on:[a]=""
                // v-on:[a.b]=""
                // v-bind:[a]=""
                // v-bind:[a.b]=""
                if (!(arg.type === 4 /* SIMPLE_EXPRESSION */ && arg.isStatic)) {
                    context.onError((0, errors_1.createMPCompilerError)(isVBind
                        ? 3 /* X_V_BIND_DYNAMIC_ARGUMENT */
                        : 1 /* X_V_ON_DYNAMIC_EVENT */, loc));
                    continue;
                }
            }
...
function processVModel(node, transformVModel, context) {
    ...
    for (let i = 0; i < props.length; i++) {
        const prop = props[i];
        if (prop.type === 7 /* DIRECTIVE */ && prop.name === 'model') {
