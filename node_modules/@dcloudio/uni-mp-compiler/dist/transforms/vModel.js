...
const transformModel = (dir, node, _context) => {
    ...
    if (dir.arg) {
        context.onError((0, compiler_dom_1.createDOMCompilerError)(55 /* X_V_MODEL_ARG_ON_ELEMENT */, dir.arg.loc));
    }
    function checkDuplicatedValue() {
        const value = (0, compiler_core_1.findProp)(node, 'value');
        if (value) {
            context.onError((0, compiler_dom_1.createDOMCompilerError)(57 /* X_V_MODEL_UNNECESSARY_VALUE */, value.loc));
        }
    }
    const { tag } = node;
    if (tag === 'input' || tag === 'textarea') {
        ...
        context.onError((0, compiler_dom_1.createDOMCompilerError)(54 /* X_V_MODEL_ON_INVALID_ELEMENT */, dir.loc));
    }
...
function findDatasetEventOpts(node) {
    ...
    if ((exp === null || exp === void 0 ? void 0 : exp.type) !== 8 /* COMPOUND_EXPRESSION */) {
        return;
    }
    for (let i = 0; i < exp.children.length; i++) {
        ...
        if (childExp.type !== 8 /* COMPOUND_EXPRESSION */) {
            continue;
        }
        ...
        if ((0, shared_1.isSymbol)(inputExp) ||
            ...
            inputExp.type !== 8 /* COMPOUND_EXPRESSION */) {
            continue;
        }
        return childExp;
    }
...
function transformVModel(props, node, context, { binding, event, formatEventCode, }) {
    if (props.length !== 2) {
        return [];
    }
    const { key: modelValueArg, value: modelValeExpr } = props[0];
    const { key: onUpdateArg, value: onUpdateExpr } = props[1];
    if (modelValueArg.type !== 4 /* SIMPLE_EXPRESSION */) {
        return [];
    }
    if (onUpdateArg.type !== 4 /* SIMPLE_EXPRESSION */ ||
    ...
    const vOnUpdate = (0, uni_cli_shared_1.createOnDirectiveNode)(event || (0, shared_1.camelize)(onUpdateArg.content.replace('onUpdate:', 'update-')), formatEventCode((0, codegen_1.genExpr)((0, vOn_1.wrapperVOn)(
    // onUpdateExpr 通常是 ExpressionNode 或者被 cache 的 ExpressionNode
    (onUpdateExpr.type === 20 /* JS_CACHE_EXPRESSION */
        ? onUpdateExpr.value
        : onUpdateExpr), node, context))));
